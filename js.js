setTimeout(function () {
  ...
  function ClickPattern(event) {
    cx = event.clientX;
    cy = event.clientY;
    if (PatAm > 0 && Errr == 0 && timerWenP == true) {
      calculatePat();
    }
  };
  ...
  function SubscriberP() {
    return {
      ...
      'date': Math.round((sc + fc) / (fc + fc + sc) * 100) + "%"
    };
  };
  ...
  function RefrePP(p) {
    ...
    Score.innerHTML = "&nbsp;";
    if (PatRec.length > 0) {
      Best.innerHTML = "HIGH SCORE<br>" + PatRec[0].record;
    } else {
      Best.innerHTML = "HIGH SCORE<br>-";
    }
    Time.innerHTML = "0";
  ...
  function Refresh(p) {
    ...
    Time.innerHTML = "30";
    if (FreRec.length > 0) {
      Best.innerHTML = "HIGH SCORE<br>" + FreRec[0].record;
    } else {
      Best.innerHTML = "HIGH SCORE<br>";
    }
    clearInterval(timerGP);
  ...
  function Subscriber() {
    return {
      'record': sc,
      'date': Math.round((sc + fc) / (fc + fc + sc) * 10000) / 100 + "%"
    };
  };
  ...
  function Timer() {
    ...
    timerGO = setInterval(function () {
      ...
      if (Bonus > 3) {
        Bonus = Math.round((Bonus - 3) * 10) / 10;
      } else {
        Bonus = 0.1;
      }
      PressKey.innerHTML = "acc: " + Math.round((sc + fc) / (fc + fc + sc) * 10000) / 100 + "%";
      //console.log(Bonus,"3");
      //console.log(Bonus);
